...
import (
	...
	"github.com/ravan/provider-orchard/apis"
	orchard "github.com/ravan/provider-orchard/internal/controller"
	"github.com/ravan/provider-orchard/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "Orchard support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-orchard"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-orchard",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Orchard APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-orchard:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(customresourcesgate.Setup(mgr, o), "Cannot setup CRD gate controller")
	kingpin.FatalIfError(orchard.SetupGated(mgr, o), "Cannot setup Orchard controllers")
